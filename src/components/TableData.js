...
class TableDataCard extends React.Component{
    ...
    render(){
        ...
        const handleOpen = (key) =>{
            ...
            console.log(key);
            axios.get(`https://cloud.iexapis.com/stock/${key}/company`)
            .then((res)=>{
                console.log(res);
            })    
        }
                ...
                return (
                    <tr key={ index }>
                        ...
                        <td className={ 
                            this.state.currency + table.c.toFixed(2) >
                            this.state.currency + table.pc.toFixed(2) ? 
                            'green-text font-weight-bold' : 'red-text font-weight-bold' }>
                                <button className="btn btn-secondary btn-search" onClick={()=>handleOpen(this.props.graphData[index].stockValue)}>Info</button>
                        </td>
                    </tr>
                );
            };
